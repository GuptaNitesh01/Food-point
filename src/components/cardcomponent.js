...
const Cardcontainer = () => {
  ...
  // const restaurantdata = [
    ...
    let data=restaurantList[1];
    let {card}=data
    let restaurant=card.card.gridElements.infoWithStyle.restaurants;
      ...
      <div className="container d-flex flex-wrap gap-5"> 
       {
          ...
          {
            restaurant.map((elements,index)=>{
                return <Restaurantcard 
                // key={Restaurantcard.info}

                imageUrl={IMG_URL + elements.info.cloudinaryImageId}
                name={elements.info.name}
                areaName={elements.info.areaName}
                avgRating={elements.info.avgRating} 
                cuisines={elements.info.cuisines} />
            })
        }
      </div>
    );
